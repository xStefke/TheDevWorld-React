    ...
    <footer className="bg-[#2D223C] text-white py-14">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 px-6">
          ...
          <ul className="flex flex-col gap-2 opacity-90">
            {usefulLinks.map((link, i) => (
              <li key={i}>
                ...
                >
                  <span className="text-(--primary-color)">›</span> {link.label}
                </a>
              </li>
            ))}
          </ul>
        ...
        <div className="flex flex-col ">
          ...
          <button className="mt-5 bg-(--primary-color) text-white rounded-[100px] transition-all duration-300 ease-in-out border border-solid border-(--primary-color) hover:bg-transparent hover:border text-center cursor-pointer w-full py-2 font-bebas text-[24px]">
            GET YOUR TICKET
          </button>
        </div>
      </div>
